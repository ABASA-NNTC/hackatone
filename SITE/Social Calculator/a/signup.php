...
if(isset($data['do_signup'])) {
    ...
    if (mb_strlen($data['family']) < 2 || mb_strlen($data['family']) > 50){
	    
	    $errors[] = "Недопустимая длина фамилии";

    }
	
	if (mb_strlen($data['patronymic']) < 2 || mb_strlen($data['patronymic']) > 50){
	    
	    $errors[] = "Недопустимая длина отчества";

    }
	
	if (mb_strlen($data['snils']) != 11){
	    
	    $errors[] = "Недопустимая длина снилса";
...
<div class="container mt-4">
	   ...
	   <!-- Форма регистрации -->
		...
		<form action="signup.php" method="post">
			...
			<input type="text" class="form-control" name="patronymic" id="patronymic" placeholder="Отчество" required><br>
			<input type="text" class="form-control" name="snils" id="snils" placeholder="Снилс"><br>
		...
		<p>Вернуться на <a href="\Social Calculator\index.html">главную</a>.</p>
			</div>
		</div>
	</div>
